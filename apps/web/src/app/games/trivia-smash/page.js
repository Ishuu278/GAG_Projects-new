...
export default function TriviaSmashGame() {
  const [iframeSrc, setIframeSrc] = useState("https://viebrain-games.b-cdn.net/trivia-smash/index.html");
  const [key, setKey] = useState(0);

  useEffect(() => {
    const api = getApiBaseValue();
    const slug = getSlugValue("cricket");
    setIframeSrc(`https://viebrain-games.b-cdn.net/trivia-smash/index.html?api=${encodeURIComponent(api)}&slug=${encodeURIComponent(slug)}`);
  }, [key]);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8" style={{ height: "calc(100vh - 80px)", marginTop: "80px" }}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-foreground">Trivia Smash</h1>
          <InGameAdminConfig
            defaultSlug="cricket"
