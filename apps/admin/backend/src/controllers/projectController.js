...
// @desc    Sync all games from GAG game portal into MySQL projects
// @route   POST /api/projects/sync-portal-games
// @access  Private
const syncPortalGamesHandler = async (req, res) => {
  const { syncPortalGames } = require('../utils/syncPortalGames');
  const result = await syncPortalGames();
  res.json(result);
};

module.exports = wrapAll({ getProjects, createProject, getProject, updateProject, deleteProject, syncPortalGamesHandler });