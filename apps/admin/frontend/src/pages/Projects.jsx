...
import { getProjects, createProject, updateProject, deleteProject, syncPortalGames } from '../api';
...
export default function Projects() {
  ...
  const handleSyncPortalGames = async () => {
    setSyncing(true);
    try {
      const res = await syncPortalGames();
      if (res.data?.success) {
        toast.success(`Synced ${res.data.totalSynced || 'all'} portal games! (${res.data.createdCount} new created)`);
      } else {
        toast.success('Portal games synchronized!');
      }
      load();
    } catch (err) {
    ...
    },
    {
      ...
      render: (r) => (
        <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSimulatorTarget(r); }}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={handleSyncPortalGames}
            disabled={syncing}
            className="flex items-center gap-1.5 rounded-xl border border-primary-500/30 bg-primary-500/10 px-3.5 py-2 text-xs font-bold text-primary-400 hover:bg-primary-500/20 transition-all shadow-sm"
            title="Auto-sync all 30+ games from the GAG game portal into MySQL"
          >
            <Sparkles size={14} className={syncing ? 'animate-spin' : 'text-primary-400'} />
            <span>{syncing ? 'Syncing...' : 'Sync Portal Games'}</span>
          </motion.button>
                ...
                >
                  ...
                  <div className="mt-5 pt-3 border-t border-line flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      <button
                        type="button"
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSimulatorTarget(p); }}
