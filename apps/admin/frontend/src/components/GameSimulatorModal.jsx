...
export default function GameSimulatorModal({ isOpen, onClose, project, onProjectUpdate }) {
  ...
  const resolveGameUrl = (p) => {
    if (!p?.slug) return 'http://localhost:3000/games';
    let base;
    if (p.allowedOrigins && p.allowedOrigins.length > 0) {
      const first = p.allowedOrigins[0].replace(/\/+$/, '');
      if (first.includes('/games/')) {
        base = first;
      } else if (first.startsWith('http://') || first.startsWith('https://')) {
        base = `${first}/games/${p.slug}`;
      }
    }
    if (!base) base = `http://localhost:3000/games/${p.slug}`;
    // Append embed=1 so the portal hides navbar/footer in iframe
    const sep = base.includes('?') ? '&' : '?';
    return `${base}${sep}embed=1`;
  };

  // Game Embed URL State
  const [embeddedUrl, setEmbeddedUrl] = useState(() => resolveGameUrl(project));
  ...
  useEffect(() => {
    if (project) {
      setEmbeddedUrl(resolveGameUrl(project));
