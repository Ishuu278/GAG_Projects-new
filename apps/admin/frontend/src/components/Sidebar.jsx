...
const Sidebar = ({ collapsed, onToggle, onNavigate }) => {
  ...
  const gamePortalUrl = import.meta.env.VITE_GAME_PORTAL_URL || 'http://localhost:3000';
  ...
  return (
    ...
    >
      ...
      <nav className="relative flex-1 space-y-1.5 overflow-y-auto px-3 py-4">
        ...
        {/* Link back to Main Mind Training Portal */}
        <a
          href={gamePortalUrl}
          target="_blank"
          rel="noopener noreferrer"
          title={collapsed ? 'Game Portal' : undefined}
          className="relative group flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10 transition-colors duration-200 mt-4 border border-emerald-500/20"
        >
          <ExternalLink size={18} className="shrink-0 text-emerald-400 group-hover:scale-110 transition-transform" />
          {!collapsed && <span className="truncate">Game Portal (App)</span>}
        </a>
      </nav>
